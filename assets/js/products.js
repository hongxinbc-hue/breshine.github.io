(function ($) {
    ...
    function productMarkup(product) {
        ...
        return '<div class="col-lg-4 col-md-6 col-xs-12 col-sm-12 witr_all_mb_30 product-card">' +
            ...
            '<div class="prot_content em_port_content"><img class="product-hover-logo" src="assets/images/logo3.png" alt="BRESHINE">' +
    ...
    function bindStaticProductTabs() {
        $('.filter_menu li').off('click').on('click', function () {
            var $tab = $(this);
            var selector = $tab.attr('data-filter');
            var $cards = $grid.children('.eportfolio_item');
            $('.filter_menu li').removeClass('current_menu_item');
            $tab.addClass('current_menu_item');
            if (!selector || selector === '*') {
                $cards.show();
                return;
            }
            $cards.hide().filter(selector).show();
        });
    }
